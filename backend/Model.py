...
def FirstLayerDMM(prompt: str = "test"):
    ...
    # Just in case the model returns nothing
    if not full_text.strip():
        return ["[ERROR] Model returned empty response."]

    # Clean up and split
    full_text = full_text.replace("\n", " ")
    split_tasks = [task.strip() for task in full_text.split(",") if task.strip()]

    # Filter based on valid funcs
    filtered = []
